...
var BaseTypography = React10.forwardRef((props, ref) => {
  const {
    variant = "p",
    theme: theme2 = "light",
    color,
  ...
  return React10.createElement(Component, { ...restProps, ref });
});
...
var InternalTypography = forwardRef3(({
  component,
  variant = "p",
  children,
  copyable,
...
var Link = forwardRef3((props, ref) => {
...
import { useContext as useContext5, useMemo, useRef as useRef3 } from "react";

// src/components/menu/CollapseTransition.tsx
import { useRef as useRef2, useEffect, useState as useState2, useCallback } from "react";
var CollapseTransition = ({
  ...
  const [height, setHeight] = useState2(isOpen ? "auto" : "0px");
  const [isInitial, setIsInitial] = useState2(true);
  const animationRef = useRef2(null);
  const prevIsOpen = useRef2(isOpen);
...
var MenuItem = ({ item, level, parentKey, fullKey }) => {
  ...
  const context = useContext5(MenuContext);
...
var Menu = ({
  ...
  const [activeKey, setActiveKey] = useState3(defaultActiveKey);
  const [openKeys, setOpenKeys] = useState3(defaultOpenKeys);
  const [menuItems, setMenuItems] = useState3([]);
  useEffect2(() => {
  ...
  const toggleOpenKey = useCallback2((key, _level, parentKey) => {
    setOpenKeys((prev2) => {
  ...
  const setOpenKey = useCallback2((key, isOpen, parentKey) => {
    const fullKey = parentKey ? `${parentKey}-${key}` : key;
    setOpenKeys((prev2) => {
      if (isOpen) {
...
var PaginationContext = createContext3(null);
...
// src/components/pagination/paginationRenderer.tsx
var PaginationItems = () => {
  const context = useContext7(PaginationContext);
  const [jumpPage, setJumpPage] = useState4("");
  const [dotsHover, setDotsHover] = useState4(null);
  if (!context) {
...
var SelectContext = createContext4(null);

// src/components/select/option.tsx
import { useContext as useContext8 } from "react";
var Option = ({ option, index, isFocused }) => {
  const context = useContext8(SelectContext);
  if (!context) {
...
var Select = (props) => {
  ...
  const [focusedIndex, setFocusedIndex] = useState5(0);
  const containerRef = useRef4(null);
  const initialValue = useMemo2(() => {
    if (value !== void 0) return value;
    if (defaultValue !== void 0) return defaultValue;
    return multiple ? [] : null;
  }, [value, defaultValue, multiple]);
  const [selectedValue, setSelectedValue] = useState5(initialValue);
  useEffect3(() => {
    if (value !== void 0) {
  ...
  const handleSelect = useCallback3((val) => {
    let newValue;
    if (multiple) {
...
import { forwardRef as forwardRef4, useState as useState6, useEffect as useEffect4, useCallback as useCallback4 } from "react";
...
var Input = forwardRef4(
  ({
    ...
    const [charCount, setCharCount] = useState6(inputValue.length);
    const [exceedLimit, setExceedLimit] = useState6(false);
    useEffect4(() => {
      if (propValue !== void 0) {
...
import { forwardRef as forwardRef5, useState as useState7 } from "react";
...
var TextArea = forwardRef6(
  ({
    ...
    const [charCount, setCharCount] = useState8(inputValue.length);
    const [exceedLimit, setExceedLimit] = useState8(false);
    ...
    const handleClear = useCallback5((e) => {
      e.stopPropagation();
      setInputValue("");
      setCharCount(0);
      if (onChange) {
...
var Rate = ({
  ...
  const [currentValue, setCurrentValue] = useState9(defaultValue);
  const [hoverValue, setHoverValue] = useState9(0);
  useEffect6(() => {
    if (propValue !== void 0) {
      setCurrentValue(propValue);
    }
  }, [propValue]);
  const handleClick = useCallback6((value) => {
  ...
  const handleMouseEnter = useCallback6((value) => {
    if (disabled || readOnly) return;
    const adjustedValue = allowHalf ? value : Math.ceil(value);
...
var Radio = React26.forwardRef(
  ({
    ...
    const groupContext = useContext9(RadioGroupContext);
...
var RadioGroup = React26.forwardRef(
  ({
...
var Switch = React27.forwardRef(
  ({
    ...
    const [internalChecked, setInternalChecked] = useState10(defaultChecked);
...
import { createContext as createContext5, useContext as useContext10, useState as useState12, useCallback as useCallback7 } from "react";

// src/components/message/MessageContainer.tsx
import React29, { useRef as useRef6, useEffect as useEffect7 } from "react";
...
var MessageProvider = ({ children }) => {
  ...
  const add = useCallback7((message) => {
    setMessages((prev2) => [...prev2, message]);
  }, []);
  const remove = useCallback7((id) => {
    setMessages((prev2) => prev2.filter((msg) => msg.id !== id));
  }, []);
  const removeAll = useCallback7(() => {
    setMessages([]);
  }, []);
...
var useMessageContext = () => {
  const context = useContext10(MessageContext);
  if (!context) {
...
import { useCallback as useCallback8, useContext as useContext11, useMemo as useMemo3 } from "react";
var useMessage = () => {
  const context = useContext11(MessageContext);
  if (!context) {
    throw new Error("useMessage must be used within a MessageProvider");
  }
  const { add, removeAll } = context;
  const generateId = useCallback8(() => `msg_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`, []);
  const messageApi = useMemo3(() => ({
...
import { createContext as createContext6, useContext as useContext12, useState as useState13, useCallback as useCallback9 } from "react";
var ModalContext = createContext6(null);
var ModalProvider = ({ children }) => {
  const [modalContent, setModalContent] = useState13(null);
  const [modalConfig, setModalConfig] = useState13({});
  const [isOpen, setIsOpen] = useState13(false);
  const [closing, setClosing] = useState13(false);
  const openModal = useCallback9((content, config = {}) => {
...
var useModalContext = () => {
  const context = useContext12(ModalContext);
  if (!context) {
...
import { useState as useState14, useRef as useRef7, useEffect as useEffect9 } from "react";
...
import { useState as useState15, useEffect as useEffect10, useRef as useRef8 } from "react";
...
var Spin = ({
  ...
  const [isSpinning, setIsSpinning] = useState15(false);
  const delayTimer = useRef8(null);
  useEffect10(() => {
    if (delay > 0) {
...
import { useEffect as useEffect11, useState as useState16, useCallback as useCallback10, useRef as useRef9 } from "react";
...
var Drawer = ({
  ...
  const [isOpen, setIsOpen] = useState16(open);
  const [isMounted, setIsMounted] = useState16(open);
  const drawerRef = useRef9(null);
  const handleTransitionEnd = useCallback10(() => {
    if (!open && destroyOnClose) {
      setIsMounted(false);
    }
  }, [open, destroyOnClose]);
  const handleMaskClick = useCallback10(() => {
...
var Tooltip = ({
  ...
  const [calculatedPosition, setCalculatedPosition] = useState17({ top: 0, left: 0 });
...
import React39, { useState as useState18, useCallback as useCallback12 } from "react";
...
var Tag = React39.forwardRef(({
  ...
  const [internalChecked, setInternalChecked] = useState18(defaultChecked);
  const [visible, setVisible] = useState18(true);
  const isChecked = controlledChecked !== void 0 ? controlledChecked : internalChecked;
  const handleClose = useCallback12((e) => {
    e.stopPropagation();
    if (onClose) {
      onClose(e);
    }
  ...
  const handleClick = useCallback12((e) => {
    if (checkable) {
...
function List({
  ...
  const [visibleRange, setVisibleRange] = useState19({ start: 0, end: 0 });
  const [dimensions, setDimensions] = useState19({ width: 0, height: 0 });
  const calculateVisibleRange = useCallback13(() => {
...
// src/components/collapse/collapse.tsx
var CollapseContext = createContext7(void 0);
var Collapse = ({
  children,
  accordion = false,
  ...
  const [activeKeys, setActiveKeys] = useState20(getInitialActiveKeys());
  useEffect14(() => {
    if (propActiveKey !== void 0) {
...
var Panel = ({
  ...
  const context = useContext13(CollapseContext);
  if (!context) {
...
var Tree = React43.forwardRef(({
  ...
  const [expandedKeys, setExpandedKeys] = useState21(defaultExpandedKeys);
  const [selectedKey, setSelectedKey] = useState21(defaultSelectedKey);
  ...
  const renderTreeNodes = (nodes, level = 0) => {
    return nodes.map((node2) => /* @__PURE__ */ jsxs2(React43.Fragment, { children: [
      /* @__PURE__ */ jsx4(
        TreeNode,
        {
          id: node2.id,
...
import { forwardRef as forwardRef7, useRef as useRef12, useEffect as useEffect15, useState as useState22, useCallback as useCallback14, useMemo as useMemo4 } from "react";
...
var Checkbox = forwardRef7(
  ({
    ...
    const [localChecked, setLocalChecked] = useState22(defaultChecked);
    const handleChange = useCallback14((e) => {
      if (disabled) return;
      const newChecked = e.target.checked;
      if (!isControlled) {
...
var CheckboxGroup = ({
  ...
  const [localValue, setLocalValue] = useState22(defaultValue);
  const selectedValues = isControlled ? controlledValue : localValue;
  const handleChange = useCallback14((value, checked) => {
    let newValues;
    if (checked) {
...
import React46, { cloneElement as cloneElement2, Children } from "react";
...
var Breadcrumb = ({
  ...
  const renderOmittedList = (omitted) => {
    return /* @__PURE__ */ jsx4("div", { style: {
      ...
      if (React46.isValidElement(item) && item.type === BreadcrumbItem) {
  ...
  const renderItems = () => {
    ...
    if (!responsive) {
      return childrenArray.map((child, index) => {
        ...
        if (React46.isValidElement(child) && child.type === BreadcrumbItem) {
    ...
    const firstEl = React46.isValidElement(first) && first.type === BreadcrumbItem ? cloneElement2(first, {
      key: first.key ?? "first",
    ...
    const secondLastEl = React46.isValidElement(secondLast) && secondLast.type === BreadcrumbItem ? cloneElement2(secondLast, {
    ...
    const lastEl = React46.isValidElement(last) && last.type === BreadcrumbItem ? cloneElement2(last, {
      key: last.key ?? "last",
      isCurrent: true,
      separator: null
    ...
    const middleEls = middleItems.map((child, index) => {
      const idx = index + 1;
      const el = React46.isValidElement(child) && child.type === BreadcrumbItem ? cloneElement2(child, {
...
import React49, { useState as useState23, useMemo as useMemo5, useCallback as useCallback15, useEffect as useEffect17 } from "react";
...
var MemoizedRow = React49.memo(RowComponent, areEqual);
var Table = ({
  dataSource = [],
  columns,
  rowKey,
  ...
  const [sortState, setSortState] = useState23({ direction: null });
  const [selectedRowKeys, setSelectedRowKeys] = useState23(rowSelection?.selectedRowKeys || []);
  const selectedRowKeysSet = useMemo5(() => new Set(selectedRowKeys), [selectedRowKeys]);
  useEffect17(() => {
  ...
  const getRowKey = useCallback15((record, index) => {
  ...
  const handleRowSelect = useCallback15((record, checked, rowIndex) => {
  ...
  const handleSelectAll = useCallback15((checked) => {
  ...
  const renderRows = useCallback15(() => {
...
import { useContext as useContext15, forwardRef as forwardRef8, useImperativeHandle, useEffect as useEffect19, useRef as useRef15, useMemo as useMemo7 } from "react";
...
var FormItem = ({
  ...
  const childNode = React50.isValidElement(children) ? React50.cloneElement(children, {
    id,
...
import { useState as useState24, useCallback as useCallback16, useRef as useRef14, useMemo as useMemo6 } from "react";
var useForm = (initialValues = {}) => {
  const [formData, setFormData] = useState24(initialValues);
  const [errors, setErrors] = useState24({});
  const fieldRules = useRef14({});
  const initialDataRef = useRef14(initialValues);
  const getFieldValue = useCallback16((name) => {
    return formData[name];
  }, [formData]);
  const getFieldsValue = useCallback16(() => {
    return { ...formData };
  }, [formData]);
  const setFieldValue = useCallback16((name, value) => {
  ...
  const setInitialValues = useCallback16((values) => {
    initialDataRef.current = values;
    setFormData(values);
  }, []);
  const setFieldsValue = useCallback16((values) => {
    setFormData((prev2) => ({ ...prev2, ...values }));
    setErrors({});
  }, []);
  const resetFields = useCallback16(() => {
    setFormData(initialDataRef.current);
    setErrors({});
  }, []);
  const registerField = useCallback16((name, rules = []) => {
    fieldRules.current[name] = rules;
  }, []);
  const validateField = useCallback16(async (name, value) => {
  ...
  const validate = useCallback16(async () => {
    const newErrors = {};
    let isValid = true;
...
var Form = forwardRef8(({
  ...
  const formRef = useRef15(form);
  formRef.current = form;
  const stableInitialValues = useMemo7(() => initialValues, [JSON.stringify(initialValues)]);
...
var Tabs = React52.forwardRef(({
  ...
  const [activeKey, setActiveKey] = useState25(
